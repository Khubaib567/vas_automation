...
const get_past_matches = (league_name,date)=> {
    ...
    request(URL,(err,res,html)=>{
        ...
        const league_name = body_tag.find('div.page-container').find('h1').text()
        
        console.log(league_name)
        const matches_container = match_div.find('div.match-row.match-row--status-pld').first()       
        if(!matches_container) return console.log("No matches have been played on this date!")
        else{
            const matches = [];
            matches_container.each((i,el)=>{
                const date = $(el).find('div.match-row__status').find('span.match-row__date').text()
                const team1 = $(el).find('table.match-row__teams').find('td.match-row__team-home').text().trim()
                const score1 = $(el).find('td:nth-child(1)').find('b.match-row__goals').text().trim()
                const team2 = $(el).find('table.match-row__teams').find('td.match-row__team-away').text().trim()
                const score2 = $(el).find('td:nth-child(3)').find('b.match-row__goals').text().trim() 
                const current_time = new Date()
                const match_details_link = $(el).find('a.match-row__link').attr('href') 
                ...
                get_match_details(match_details_link).then((get_details)=>{
                    // const team1_badge_url = get_details.team1_badge
                    // const team2_badge_url = get_details.team2_badge
                    // const match = {match_date:date,team1_name:team1,team1_score:score1,team1_badge_url:team1_badge_url,team2_name:team2,team2_score:score2,team2_badge_url:team2_badge_url,time_stamp:current_time}
                    // matches.push(match)
...
const get_next_matches = (league_name,date)=>{
    ...
    request(getURL,(err,res,html)=>{
        ...
        console.log(league_name)
        const matches = container.find('div.container-competition-matches').find('div.widget-competition-matches').find('div.match-row.match-row--status-fix')
        const date = matches.find('div.match-row__status').find('span.match-row__date').text()
        if(!date) return console.log("No fixtures have been found on this date!")
        else{
            matches.each((i,el)=>{
                const date = $(el).find('div.match-row__status').find('span.match-row__date').text()
                const team1 = $(el).find('table.match-row__teams').find('td.match-row__team-home').text()
                const team2 = $(el).find('table.match-row__teams').find('td.match-row__team-away').text()
                 console.log(`\n     ${date}\n${team1}-${team2}`)
            })
        }
...
const get_match_summary = (league_name,date)=> {
    ...
    request(URL,(err,res,html)=>{
        ...
        console.log(league_name)
        // match_summary.push(league_name)
        const matches_container = match_div.find('div.match-row.match-row--status-pld')       
        
        const date = matches_container.find('div.match-row__status').find('span.match-row__date').text()
        if(!date) return console.log("No matches have been played on this date!")
        else{
            let links_arr = []  // IT GIVES THE LINKS ORDER WISE
            matches_container.each((i,el)=>{
                const match_details_link = $(el).find('a.match-row__link').attr('href') 
            ...
            let interval = setInterval(()=>{
                if(counter > links_arr.length-1){
                    ...
                    get_summary(links_arr[counter]).then((summary)=>{
                        team2_details=summary.team2_goals
                        team2_name = team2_details[0].name
                        first_goal = team2_details[1][0]
                        second_goal = team2_details[1][1]

                        console.log(team2_name,first_goal,second_goal)
                        // const team1 = summary.team1.name
                        // const team1_score = summary.team1.score
                        // console.log(team1,team1_score)
                        // const teams_score = summary.teams_score
                        // const team2 = summary.team2.name
                        // const team2_score = summary.team2.score

                //         fs.readFile('yesterday_summary.csv', 'utf8', function(err, data){
                //             if(data == undefined){  
                //                 const writeStream = fs.createWriteStream('yesterday_summary.csv')
                //                 // Write Headers
                //                 writeStream.write(`Team_1,Final_Score,Team_2\n`)
                //                 writeStream.write(`${team1},${teams_score},${team2}\n`)
                //                 console.log('File has saved!');
                //             }else{
                //                 fs.appendFileSync('yesterday_summary.csv',`${team1},${teams_score},${team2}\n`)
...
module.exports = {get_live_match,get_past_matches,get_next_matches,get_match_summary}
